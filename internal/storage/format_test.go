...
func TestGenericBrowserHTMLImport(t *testing.T) {
	input := `<!DOCTYPE NETSCAPE-Bookmark-file-1><DL><p><DT><H3>工作</H3><DL><p><DT><A HREF="https://example.com" TAGS="资料,待读">Example</A><DD>备注</DL><p></DL><p>`
	state, err := DecodeHTML([]byte(input))
	if err != nil {
		t.Fatal(err)
	}
	if len(state.Groups) != 1 || len(state.Bookmarks) != 1 || state.Bookmarks[0].Notes != "备注" || state.Bookmarks[0].GroupID != state.Groups[0].ID {
