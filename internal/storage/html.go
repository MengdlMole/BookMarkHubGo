...
func writeHTMLLevel(out *bytes.Buffer, state model.State, parentID string, depth int) {
	...
	for _, bookmark := range state.Bookmarks {
		...
		out.WriteString(fmt.Sprintf("%s<DT><A HREF=\"%s\" ADD_DATE=\"%d\" LAST_MODIFIED=\"%d\" TAGS=\"%s\" DATA-BOOKMARKHUB-ID=\"%s\" DATA-BOOKMARKHUB-REVISION=\"%s\">%s</A>\n",
			indent, attr(bookmark.URL), added, modified, attr(strings.Join(bookmark.Tags, ",")), attr(bookmark.ID), attr(bookmark.Revision), html.EscapeString(bookmark.Title)))
		if bookmark.Notes != "" {
		...
		case xml.EndElement:
			...
			if current != nil && name == current.kind {
				...
				case "a":
					...
					state.Bookmarks = append(state.Bookmarks, model.Bookmark{ID: id, URL: current.attrs["href"], Title: first(text, current.attrs["href"]), GroupID: groupID, Tags: model.NormalizeTags([]string{current.attrs["tags"]}), CreatedAt: created, UpdatedAt: updated, Revision: revision})
